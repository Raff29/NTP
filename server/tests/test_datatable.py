...
class TestDataTableRoutes:
    def test_get_instruction_logs(self, test_client, test_database):
        ...
        instruction_log = InstructionLog(
            user_id=user_id, filename='test.txt', instructions='Test instruction', is_archived=False)
    ...
    def test_update_instruction_logs(self, test_client, test_database):
        ...
        # Act
        response = test_client.put(url_for(
            'instruction_logs.update_instruction_logs',
            id=original_instruction_log.id), json=updated_data)

        # Assert
        assert response.status_code == 200

        updated_instruction_log = test_database.session.get(InstructionLog,
                                                            original_instruction_log.id)
    ...
    def test_archive_instruction_logs(self, test_client, test_database):
        # Arrange
        instruction_log = InstructionLog(
            user_id=1, filename='test.txt', instructions='Test instruction', is_archived=False)
        test_database.session.add(instruction_log)
        test_database.session.commit()
    ...
    def test_delete_instruction_logs(self, test_client, test_database):
        ...
        # Act
        response = test_client.delete(url_for(
            'instruction_logs.delete_instruction_logs',
            user_id=1, id=instruction_log.id),
            json={'is_admin': True})
